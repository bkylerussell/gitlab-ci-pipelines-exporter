...
// EditProjectHook ..
func (c *Client) EditProjectHook(ctx context.Context, projectName string, hid int64, options *goGitlab.EditProjectHookOptions) (hook *goGitlab.ProjectHook, err error) {
...
// RemoveProjectHook ..
func (c *Client) RemoveProjectHook(ctx context.Context, projectName string, hookID int64) (err error) {
	ctx, span := otel.Tracer(tracerName).Start(ctx, "gitlab:RemoveProjectHook")
	defer span.End()
	span.SetAttributes(attribute.String("project_name", projectName))
	span.SetAttributes(attribute.Int64("hook_id", hookID))
